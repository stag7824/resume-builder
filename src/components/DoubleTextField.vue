    ...
    <el-row class="mt-2" align="middle">
      <el-col :span="24">
        <TextField v-if="firstFieldLabel && firstFieldName"
            :id="`${panelName.toLowerCase()}-${firstFieldName.toLowerCase()}-${id}`"
            :name="firstFieldName"
            :label="firstFieldLabel"
            v-model="componentData[`${firstFieldName}`]"
            @input="onInput" />
  
        <TextField v-if="secondFieldLabel && secondFieldName"
            :id="`${panelName.toLowerCase()}-${secondFieldName.toLowerCase()}-${id}`"
            :name="secondFieldName"
            :label="secondFieldLabel"
            v-model="componentData[`${secondFieldName}`]"
            @input="onInput" />
